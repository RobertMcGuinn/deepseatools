...
filename <- "20250324-3_NOAA_CBNMS_Delta_2002_2003_142456"
...
## because no synonyms this code is to add dummy dataframe.
synonyms <- vernaculars
synonyms <- synonyms %>%
  rename(synonyms_list = vernaculars_list)
...
##### check #####
# sub_enhanced3 %>%  pull(Phylum) %>% table()
# sub_enhanced3 %>%  pull(ScientificName) %>% table()
# sub_enhanced3 %>%  pull(AphiaID) %>% table()
...
##### load data #####
setwd('c:/rworking/deepseatools/indata')
filename <- '20250331-1_NOAA_CBNMS_Delta_2002_2003_142456'
sub <- read.csv(paste(filename, '.csv', sep = ''))

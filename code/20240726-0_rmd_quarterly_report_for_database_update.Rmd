...
## [Manual]
quarter <- 'Q3, FY-2024'
version <- 'DSCRTP_NatDB_20240726-0'
...
indata1<-read.csv("DSCRTP_NatDB_20230928-0.csv", 
                  header = T, 
                  encoding = 'latin9')
filt1 <- indata1 %>%
  filter(Flag == "0")
...
##### bring new db in #####
setwd("C:/rworking/deepseatools/indata")
indata2<-read.csv("DSCRTP_NatDB_20240325-0.csv", 
                  header = T, 
                  encoding = 'latin9')
filt <- indata2 %>%
  filter(Flag == "0")
...
## pick out the new and 
newdatasetIDs <- filt %>% 
  filter(as.Date(EntryDate) > as.Date('2023-09-01')) %>%  
  pull(DatasetID) %>% 
  unique()

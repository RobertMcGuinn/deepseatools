...
# write all shapefiles to folder (does not need to run every time)

```{r}

fgdb_path <- 'C:/data/aprx/protected_layers_2021/protected_layers_2021.gdb'

## caribbean
arc.write(file.path(fgdb_path, 'grammanik'), data=grammanik, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'hind'), data=hind, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'mutton'), data=mutton, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'redhind'), data=redhind, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'navassa'), data=navassa, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'bajodesico'), data=bajodesico, overwrite = TRUE)

## southeast
arc.write(file.path(fgdb_path, 'stetson_miami'), data=stetson_miami, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'oculinabankoeca'), data=oculinabankoeca, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'mpaIMS'), data=mpaIMS, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'oculinabankold'), data=oculinabankold, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'oculinabank'), data=oculinabank, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'pourtales_minus_crab'), data=pourtales_minus_crab, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'capefear'), data=capefear, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'capelookout'), data=capelookout, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'capelookout_dissolve'), data=capelookout_dissolve, overwrite = TRUE)

## hapc (GOMEX)
arc.write(file.path(fgdb_path, 'hapc_c9nr'), data=hapc_c9nr, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'hapc_c9r'), data=hapc_c9r, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'hapc_ewor'), data=hapc_ewor, overwrite = TRUE)
arc.write(file.path(fgdb_path, 'hapc_ewr'), data=hapc_ewr, overwrite = TRUE)
...
# write points to geodatabase  

```{r}

sub2 <- sub %>% dplyr::select(Longitude, Latitude, sedci, ScientificName)
...
arc.write(file.path(fgdb_path, 'geosub'), data=geosub, overwrite = TRUE)

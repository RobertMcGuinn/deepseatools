...
ggplot() +
  geom_sf(data = oculina, fill = "blue", alpha = 0.3) +
  geom_sf(data = sfaa, fill = "red", alpha = 0.3) +
  geom_sf(data = sfaa_buf_20000, fill = "black", alpha = 0.3) +
  geom_sf(data = sfaa_buf_5000, fill = "green", alpha = 0.3) +
  geom_sf(data = points_in_0_select) +
  geom_sf(data = points_in_5000_select) +
  geom_sf(data = points_in_20000_select) +
  theme_minimal()

```

```{r load_images_to_local_folder, eval=F, echo=F}
...
library(spocc)
out <- occ(geometry = sfaa_transform, from = 'obis', limit = 10000)
...
```{r get_obis_data_in_5000m_buffer, echo=F}
...
library(spocc)
out <- occ(geometry = sfaa_buf_5000_transform, from = 'obis', limit = 10000)
...
# using bounding box variables defined above
library(spocc)
out <- occ(geometry = sfaa_buf_20000_transform, from = 'obis', limit = 10000)
...
```{r get_data_from_gbif, cache=T, eval=F, echo=F}

##### get data from GBIF #####
out <- occ(geometry = sfaa_transform, from = 'gbif')

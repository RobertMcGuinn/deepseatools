---
title: "Area of Interest Report - Hudson Canyon Region"
...
gebco_2024_bathy <- rast("C:/rworking/deepseatools/indata/gebco_2024_n41.0_s36.0_w-75.0_e-65.0.tif")
...
```{r create_variables, echo=F, eval=T}
##### create variables ##### 
db_version <- unique(filt$DatabaseVersion)
aoi_name <- "AOI Name Here"
drive_link <- "https://drive.google.com/drive/folders/1g6kNh7mY05i8PvUwrSN_z6blKYEiJM5A?usp=drive_link"
...
```{r create_aoi, echo=FALSE, echo = F, message = FALSE, warning=FALSE, results = "hide", eval=T }
...
aoi <- protected_areas %>% filter(Sitename == 'Frank R Lautenberg (Combined Areas)')

##### reproject to target crs ##### 
aoi_5070 <- st_transform(aoi, crs = 5070)

##### Write to (for mapping) #####
# aoi_5070_spat <- vect(aoi_5070)
# writeVector(aoi_5070_spat, "C:/rworking/deepseatools/indata/aoi_5070.shp", overwrite = TRUE)
...
aoi_buf_5000 <- st_buffer(aoi_5070, dist=5000) # map units are in meters

##create 20000 meter buffer
aoi_buf_20000 <- st_buffer(aoi_5070, dist=20000) # map units are in meters
...
```{r intersect_points_with_aoi, echo=F, eval = T}
##### intersection operations ##### 
## transform CRS for coral and sponge points projection to match polygons
points_transform <- st_transform(points, crs = st_crs(aoi_5070))

## checking
##  st_crs(points_transform)

## find points within the polygons
points_in_0 <- st_join(points_transform, aoi_5070, join = st_within) # no buffer
points_in_5000 <- st_join(points_transform, aoi_buf_5000, join = st_within) # 5000 meter buffer
points_in_20000 <- st_join(points_transform, aoi_buf_20000, join = st_within) # 20000 meter buffer
...
points_in_0_select <- points_in_0 %>% filter(is.na(Sitename) == F)
points_in_5000_select <- points_in_5000 %>% filter(is.na(Sitename) == F)
points_in_20000_select <- points_in_20000 %>% filter(is.na(Sitename) == F)
...
To create summaries of occurrence data to support decision making in the deep-sea regions of `r aoi_name`.  

# Resources

- [Google drive folder containing inputs, code, and reports](`drive_link`)

- [RMarkdown code for generating this report](https://github.com/RobertMcGuinn/deepseatools/blob/master/code/mod_ndb_aoi.Rmd) 

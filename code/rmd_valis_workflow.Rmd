...
d$DepthCat4[d$DepthInMeters < 150] <- "< 150 m"
d$DepthCat4[d$DepthInMeters > 150 & d$DepthInMeters <= 300] <- "150-300 m"
d$DepthCat4[d$DepthInMeters > 300 & d$DepthInMeters <= 600] <- "300-600 m"
d$DepthCat4[d$DepthInMeters > 600] <- "> 600 m"
...
d2 <- d %>%
  filter(Genus %in% genera, 
         Phylum == "Cnidaria", 
         Ecoregion == 'North Carolinian',
         as.numeric(DepthInMeters) < 1250 )

g <- ggplot(d2, aes(reorder(Genus, DepthInMeters, FUN=median), as.numeric(DepthInMeters))) +   
  geom_boxplot() +
  scale_y_reverse() +
  ylab("Depth (meters)") + 
  xlab("Genus") +
...
set.seed(2)
pie<-pie(rep(1, length(sample(big_palette)), col = sample(big_palette) , main="")) 
...
# filter to those genera where we have greater than 30 records
sum_tbl <- sum_tbl %>%
  filter(Records > 30)
...
d2 <- as.data.frame(d) %>%
  filter(Genus %in% sum_tbl$Genus, 
         Phylum == "Cnidaria", 
         Ecoregion == 'South Carolinian',
         #DepthCat == 'very shallow'
         as.numeric(DepthInMeters) < 1250 )

# 'fill = Order' below is optional and you can control your pallette
...
g <- ggplot(d2, aes(factor(Genus), as.numeric(DepthInMeters))) +   
  geom_violin() +
  scale_y_reverse() +
  ylab("Depth (meters)") + 
  xlab("Genus") +
...
g <- ggplot(d2, aes(factor(Genus), as.numeric(DepthInMeters))) +   geom_violin() +
  scale_y_reverse() +
  ylab("Depth (meters)") + 
  xlab("Genus") +
  facet_wrap(~Order) +
...
library(vegan)
site.sp <- cast(d2, Ecoregion + DepthCat4 ~ Genus, 
...
site.sp2 <- cast(d2, Ecoregion + DepthCat4 ~ Genus, 
                 value='IndividualCount',
                 sum)

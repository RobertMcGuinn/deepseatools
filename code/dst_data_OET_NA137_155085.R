...
unique(sub$Vessel)
unique(sub$SurveyID)
unique(sub$DataProvider)
min(as.Date(sub$ObservationDate))
max(as.Date(sub$ObservationDate))
unique((sub$DataContact))
unique(sub$RecordType)
sub %>% filter(is.na(ImageFilePath) == F) %>% pull(ImageFilePath) %>% unique() %>% length()
...
# filt %>% filter(Family == 'Astrogorgiidae') %>%
...
#
# sub_enhanced3 %>%
#   group_by(VerbatimScientificName, ScientificName,VernacularNameCategory, AphiaID, Phylum, Class, Order, Family, Genus, Species, ScientificNameAuthorship) %>%
#   summarize(n=n()) %>% View()
#
# x <- setdiff(sub$CatalogNumber, sub_enhanced3$CatalogNumber)
# sub %>% filter(CatalogNumber %in% x) %>% pull(ScientificName) %>% unique()

# sub_enhanced3 %>% filter(ScientificName == 'Scleralcyonacea') %>% pull(VernacularNameCategory) %>% table()
# filt %>% filter(ScientificName == 'Scleralcyonacea') %>% pull(VernacularNameCategory) %>% table()
#
# sub %>% filter(ScientificName == 'Scleralcyonacea') %>% pull(IdentificationComments) %>% unique()
# sub_enhanced3 %>% filter(ScientificName == 'Caryophylliidae') %>% pull(VernacularNameCategory) %>% table()
# filt %>% filter(ScientificName == 'Primnoidae') %>% pull(VernacularNameCategory) %>% table()

# View(sub_enhanced3)

##### set some specific VernacularNameCategories #####
sub_enhanced3 <- sub_enhanced3 %>%
  mutate(
    VernacularNameCategory = case_when(
      ScientificName == 'Scleralcyonacea' ~ "gorgonian coral",
      ScientificName == 'Caryophylliidae' ~ "stony coral (cup coral)",
      TRUE ~ VernacularNameCategory  # Keep original name if no match
    ))
##### check #####
# sub_enhanced3 %>%
#   filter(VernacularNameCategory == '') %>%
...
##### fix all of the null and blanks in VernacularName, set to 'NA' #####
sub_enhanced3 <- sub_enhanced3 %>%
  mutate(VernacularName = ifelse(trimws(VernacularName) == "", 'NA', VernacularName))

##### check #####
table(sub_enhanced3$VernacularName, useNA = 'always')

##### export result to csv (export to CSV) #####
filename <- '20260313-3_OET_NA137'
...
dest_folder <- as_id("1paE-p2fcYWaS0FVM74lO_io15b5an-yW")

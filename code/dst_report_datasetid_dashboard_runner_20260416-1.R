...
## Indicate the sheetid that points to the most updated DatasetID key
sheetid <- '13eejDcwOzYWJUBeMVsnqaRT13D-50sqgZRTmJQpwEPU'

version <- as.character(unique(filt$DatabaseVersion))

##### load most current DatasetID key from local path -- OR -- #####
...
filt$CitationMaker <- paste(filt$DataProvider,'. ',
                            min(filt$ObservationDate),
                            ' to ',
                            max(filt$ObservationDate),
                            '. ',
...
# Define the base path to keep things organized
base_path <- "C:/rworking/deepseatools/reports/datasetid"

# List of subfolders to create/overwrite
subfolders <- c("cruise", "literature", "program", "repository")

# Loop through and handle the overwrite logic
for (folder in subfolders) {
  full_path <- file.path(base_path, folder)

  # 1. Delete the folder and everything inside it if it exists
  if (dir.exists(full_path)) {
    unlink(full_path, recursive = TRUE, force = TRUE)
  }

  # 2. Create the folder fresh
  # recursive = TRUE ensures parent folders are created if they don't exist
  dir.create(full_path, recursive = TRUE)
}

message("Folders successfully overwritten at: ", base_path)

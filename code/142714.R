...
##### linkage #####
filename <- '142714' ## manual: for this code file name, match to redmine
...
filename <- "20250331-0_Hourigan_NewRecords_2025Q2_142714"
...
sub2 <- sub1  %>%
  mutate(  ScientificName2 = case_when(
           ScientificName == "cf. Eleutherobia" ~ "Eleutherobia",
           ScientificName == "Chrysogorgia cf. geniculata" ~ "Chrysogorgia",
           ScientificName == "Hemicorallium cf. imperiale" ~ "Hemicorallium",
           ScientificName == "Imbricacis cf. squamata" ~ "Imbricacis",
           ScientificName == "Dendrophyliidae" ~ "Dendrophylliidae",

           TRUE ~ ScientificName  # Keep original name if no match
         ))

##### check #####
sub2 %>% group_by(ScientificName, ScientificName2) %>% summarize(n=n())
...
##### check #####
# names(aphia)
length(aphia$CatalogNumber)
length(sub$CatalogNumber)
setdiff(sub$CatalogNumber, aphia$CatalogNumber)
setdiff(aphia$CatalogNumber, sub$CatalogNumber)
...
##### load data #####
setwd('c:/rworking/deepseatools/indata')
filename <- '20250331-2_Hourigan_NewRecords_2025Q2_142714'
sub <- read.csv(paste(filename, '.csv', sep = ''), stringsAsFactors = FALSE)

##### check #####
setdiff(sub$DatasetID, filt$DatasetID)
# table(sub$Data)
...
folderurl <- "https://drive.google.com/drive/folders/1Vkmxz8nIKWKQLBkX_xB4SxKkF8-ACPdl"

...
## startdate:20250313
## purpose:ingest for 'MCMI_Benthic_Survey_2015_2022_142706'

##### linkage #####
filename <- '142706' ## manual: for this code file name, match to redmine
...
filename <- "20250320-3_MCMI_Benthic_Survey_2015_2022_142706"
...
sub2 <- sub1  %>%
  mutate(ScientificName = trimws(ScientificName),
         ScientificName <- iconv(ScientificName, to = "latin-9"),
         ScientificName2 = case_when(
           ScientificName == "unidentified Osteichthyes" ~ "Osteichthyes",
           ScientificName == "Halichonidra panicea" ~ "Halichondria (Halichondria) panicea",
           ScientificName == "Halichondria panicea" ~ "Halichondria (Halichondria) panicea",
           TRUE ~ ScientificName  # Keep original name if no match
         ))

## remove sp.
sub2 <- sub2 %>%
  mutate(ScientificName2 = str_remove(ScientificName2,  " sp\\.?$"))
...
##### check #####
sub_enhanced %>%  filter(VerbatimScientificName == "Aaptos mucronatus n.sp.") %>% pull(VerbatimScientificName)
...
##### check #####
sub_enhanced3 %>%
  group_by(VerbatimScientificName, ScientificName, Genus) %>%
  summarize(n=n()) %>% View()
...
##### check #####
sub_enhanced3 %>% group_by(VernacularNameCategory, VerbatimScientificName, ScientificName) %>%
  summarize(n=n()) %>% View()

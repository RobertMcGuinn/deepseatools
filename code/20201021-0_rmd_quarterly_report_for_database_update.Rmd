...
indata1<-read.csv("DSCRTP_NatDB_20200710-2.csv", header = T)
filt1 <- indata1 %>%
  filter(Flag == "0")

setwd("C:/rworking/deepseatools/indata")
indata2<-read.csv("DSCRTP_NatDB_20201021-0.csv", header = T)
filt <- indata2 %>%
  filter(Flag == "0")
# 
# rm(indata1)
...
# check
a <- length(setdiff(unique(x$DatasetID), unique(filt1$DatasetID)))

```

```{r manual_set_variables, echo=FALSE}
quarter <- 'Q4, FY-2020'
version <- 'DSCRTP_NatDB_20201021-0'

...
## startdate:20250313
## purpose:ingest for 'OET_NA116_2019_142459'

##### linkage #####
filename <- '142459' ## manual: for this code file name, match to redmine
...
filename <- '20250127_Graiff_OET_NA116.xlsx'
...
table(sub$Condition, useNA = "always")
...
##### ***** NEW VERSION 20250320-3 *****  #####
##### load dataset from CSV #####
filename <- "20250321-3_OET_NA116_2019_142459"
...
## trim whitespace
sub2 <- sub1  %>%
  mutate(ScientificName2 = trimws(ScientificName))

## remove sp. and spp.
sub2 <- sub2 %>%
  mutate(ScientificName2 = str_remove(ScientificName2, " spp?\\.?$"))
sub2 <- sub2 %>%
  mutate(ScientificName2 = str_remove(ScientificName2, " spp. #2"))
sub2 <- sub2 %>%
  mutate(ScientificName2 = str_remove(ScientificName2, " sp. 1"))
sub2 <- sub2 %>%
  mutate(ScientificName2 = str_remove(ScientificName2, " spp. #1 "))
sub2 <- sub2 %>%
  mutate(ScientificName2 = str_remove(ScientificName2, " spp. #1"))
sub2 <- sub2 %>%
...
##### load data #####
setwd('c:/rworking/deepseatools/indata')
filename <- '20250328-1_OET_NA116_2019_142459'
sub <- read.csv(paste(filename, '.csv', sep = ''))
...
##### run QA report and post to Google Drive#####
...
folderurl <- "https://drive.google.com/drive/folders/1R12H5JNjQ5ePcaktaWb_lI-bcu7QM2nT"
...
##### check #####
View(x)
filt %>% filter(grepl('OET_NA116', DatasetID)) %>% pull(SurveyID) %>% unique()
sub %>% filter(grepl('OET', DatasetID)) %>% pull(DatasetID) %>% unique()
sub %>% filter(grepl('OET', DatasetID)) %>% pull(SurveyID) %>% unique()
sub$SurveyID
sub$DataContact

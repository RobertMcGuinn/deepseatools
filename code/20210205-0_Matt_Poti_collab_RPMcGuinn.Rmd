...
## checking: 
x <- setdiff(poti, filt$DatasetID)
x <- filt %>% filter(grepl("Battista", PI)) %>% pull(DatasetID) %>% unique()
x <- filt %>% filter(grepl("Nancy Foster", Vessel)) %>% pull(DatasetID) %>% unique()
x <- filt %>% 
  filter(grepl("Nancy Foster", Vessel)) %>% 
  group_by(DatasetID, SurveyID) %>% summarize(n=n()) %>% View()

x <- filt %>% 
  filter(grepl("NF", SurveyID) | 
           grepl("Nancy Foster", Vessel) | 
           grepl('Battista', PI) | 
           grepl('Reiss', PI) |
           grepl('Battista', Citation)
           ) %>% 
...
x <- filt %>% 
  filter(grepl("SOI", DatasetID) | 
           grepl("Falkor", Vessel)
           ) %>% 
  group_by(DatasetID, SurveyID, AccessionID) %>% 
  summarize(n=n())
View(x)

x <- filt %>% 
  filter(grepl("NOAA_PC-11-05-L", DatasetID)
           ) %>% 
  group_by(DatasetID, SurveyID, EventID, ObservationDate, Citation, WebSite) %>% 
  summarize(n=n())
View(x)

x <- filt %>% 
  filter(grepl("998933", CatalogNumber)
           ) %>% 
  group_by(ScientificName, ImageURL) %>% 
  summarize(n=n())
View(x)
...
# set 'EcoRegion' variable via latitude bounds
```{r, echo=F}
x <- x %>%
  mutate(
    EcoRegion = case_when(

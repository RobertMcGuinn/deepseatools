---
title: "Area of Interest Report - Aleutian Islands"
...
```{r load_NDB, echo=F, message=FALSE, warning=FALSE, cache=F, eval=F}
## load NDB
...
aoi_name <- "Aleutian Islands (overall)"
...
```{r load_dives, echo=F, message = FALSE, warning = FALSE, cache = TRUE, eval = F}
##### load dives ##### 
digits = 12
...
```{r plot_aois, echo=FALSE, echo = F, message = FALSE, warning=FALSE, results = "hide", eval=T }
...
```{r transform_point_crs, echo=F, eval = T}
## transform CRS for coral and sponge points projection to match polygons
points_transform <- st_transform(points, crs = st_crs(aoi_transform))
...
- [RMarkdown code for generating this report](`r  paste0('https://github.com/RobertMcGuinn/deepseatools/blob/master/code/', v,'_',filename,'.rmd')`)
...
Coral, sponge, and fish occurrences from NOAA's National Database for Deep-sea Corals and Sponges are summarized within the `r aoi_name`. The database version used in this report is `r unique(filt$DatabaseVersion)`.

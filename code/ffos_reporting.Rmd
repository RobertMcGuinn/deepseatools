---
title: 'FFO'
author: Susan Gottfried, Robert McGuinn (forked on 20200403)
output: 
    word_document:
...
```{r read, echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE}
...
ffos <- ffos[apply(ffos, 1, function(y) !all(is.na(y))),]
...
```{r report, echo=FALSE, message=FALSE, results="asis"}

for(i in unique(ffos_data$FiscalYear)) {
  cat("  \n# Fiscal Year:", i, "\n")
  cat("  \n## Projects:","\n")
  x <- ffos_data %>% filter(FiscalYear == i) %>% 
    dplyr::select(-FiscalYear, )
  print(kable(x, format = 'markdown'))
  cat("  \n### Total GB:", sum(x$VolumeInGB_round), "\n" )
  cat("#####  ")
}

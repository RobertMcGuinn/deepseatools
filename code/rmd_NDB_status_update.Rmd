...
#### Figure 5: Distribution of 'IndividualCount' values between 0 and 50 corals or sponges. The figure's x-axis is truncated at 50 individuals for visual clarity, but the true maximum for 'IndividualCount' is `r prettyNum(max, big.mark = ",")`
...
#### Figure 6a: Histogram of minimum size values. For clarity the x-axis limit has been set to 100 cm.
...
#### Figure 6b: Histogram of maximum size values. For clarity the x-axis limit has been set to 100 cm.
...
Approximately `r round((y/t) *100,.1)`% of all records have 'Condition' values reported. The currently valid values for Condition are: `r s %>% filter(FieldName == 'Condition') %>% dplyr::select(ValidValues)`. Most of the records that have 'Condition' reported have been reported as 'Live'. It is likely that this is influenced by a strong search and observation bias for living coral and sponges, rather than any real prevalence of living vs. dead or damanged coral (**Figure 7**).  
...
All records in the database are populated with a complete taxonomic string. In addition, greater information about taxonomic identifications are available in the following descriptive fields: `r z`. See **Appendix 2** for the full definitions of these fields.  
...
`r round((y/t) *100,1)`% of all records (n = `r prettyNum(y, big.mark = ',')`) have 'RecordType' values reported. See **Figure 9** below for a view of the distribution of records by 'RecordType' within each Fishery Management Council region. **Figure 10** shows the distribution of these values across the entire database, regardless of region.
...
yo <- filt %>% 
  filter(SamplingEquipment %in% c('ROV', 'AUV', 'submersible', 'drop camera', 'towed camera', 'trawl', 'net', 'dredge', 'longline', 'pot', 'hook and line', 'grab', 'corer', 'SCUBA', 'epibenthic sled', 'other', 'NA'))

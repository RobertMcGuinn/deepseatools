...
setwd("C:/rworking/deepseatools/indata")
fileout <- "table.docx" # uncomment to write in your working directory
...
ft <- flextable(x) %>% 
  ...
  set_header_labels(Field_Names = "Field Name", n = "# of Fields", DSCRTPCategory = "DSCRTP Category") %>% 
  colformat_num(col_keys = colkeys, digits = 0, big.mark = ",", na_str = "NA") %>% 
  font(fontname = "Cambria", part = "all")  %>% #header, body, all
...
Taxonomic identifications can change through time, either through refinements in identification by taxonomists or through changes to underlying group taxonomy. DSCRTP attempts to keep up with these changes, but these fields should be checked carefully before using the data for analysis.

...
4. Mississippi State University, Northern Gulf Institute, NCEI Affiliate (NGI)
...
The National Oceanic and Atmospheric Administration (NOAA) Deep Sea Coral Research and Technology Program (DSCRTP), herein referred to as the 'Program', compiles and curates a National Database of the known locations of deep sea corals and sponges. The database contains observations of places where corals and sponges occur (occurrences). The database focuses on occurrences within the United States Exclusive Economic Zone (US-EEZ), but the Program includes records outside of the US-EEZ where available and is actively encouraging partnerships worldwide to enhance the geographic scope of the data served. The database fulfills NOAA's requirements under the reauthorized Magnuson-Stevens Fishery Conservation and Management Act (MSA, 2007) to identify and map locations of deep sea corals, and to submit this information for use by Regional Fishery Management Councils (FMCs). See http://www.fisherycouncils.org for an overview of the structure and function of the FMCs. The Program intends for this database to be the most comprehensive, quality-assured, national-scale, data portal for deep sea corals and sponges available (Hourigan et al., 2015).
  
The database was originally conceived in 2009, building upon an initial United States Geological Survey Cold-Water Coral Database that was limited to Atlantic records (Scanlon et al., 2010). Through the sustained partnership and funding activities of the DSCRTP, and the contributions of `r length(unique(factor(filt$DataProvider)))` unique data providers, this database now includes over `r prettyNum(a, big.mark = ",")` occurrences of deep sea corals and sponges world-wide (at database version: `r version`).  The database is now being used as an essential trusted input to open ocean planning, management, and conservation activities at the Regional Fishery Management Councils and beyond.  
  
    
The objectives of this technical memo are to:
  
* Acknowledge the many contributors to the database (as of the date of this publication).  
* Identify recent enhancements and additions to the database and related data products.  
...
The National Oceanic and Atmospheric Administration (NOAA) Deep Sea Coral Research and Technology Program (DSCRTP), herein referred to as the ‘Program', compiles and curates a National Database of the known locations of deep sea corals and sponges. The database contains observations (occurrences) of places where corals and sponges have been observed during research and in some cases includes metrics of size, abundance, and environmental conditions at the time of observation. The database focuses on occurrences within the United States (US) Exclusive Economic Zone (EEZ), but the Program includes records outside of the US-EEZ where available and is actively encouraging partnerships worldwide to enhance the geographic scope of the data served. The database fulfills NOAA's requirements under the reauthorized Magnuson-Stevens Fishery Conservation and Management Act (MSA, 2007) to identify and map locations of deep sea corals, and to submit this information for use by Regional Fishery Management Councils (FMCs). See http://www.fisherycouncils.org for an overview of the structure and function of the FMCs. The Program intends for this database to be the most comprehensive, quality-assured, national-scale, data portal for deep sea corals and sponges available (Hourigan et al., 2015).

The database was originally conceived in 2009, building upon an initial United States Geological Survey Cold-Water Coral Database that was limited to Atlantic records (Scanlon et al., 2010). Since 2012, the database has undergone rapid development experiencing periods of exponential growth during major data harvests (*Figure 1*). From 2012 to 2015 was a period of internal growth and development of the database, including the establishment of new field names, definitions, and accepted (valid) values. Additionally, a web-based mapping and data download site was established and is available at DeepSeaCoralData.NOAA.gov. Online access to the database, through the DSCRTP map portal, began in the fall of 2015. Growth from 2015 and onward was marked by quarterly data updates made available to the public. The quarterly database updates include the addition of new occurrences as well as improvements to existing records.
  
The database includes `r bb` publically available fields that relate to the location, taxonomy, environmental conditions, and the provenance of the observations. All of the values in this report are based on version `r version` of the National Database, thus some of the reported values will change as the database grows and changes through time. There are currently over `r length(unique(factor(filt$DataProvider)))` individual records in the database composed of 226 individual data sets from 75 different data providers.  See *Table 1* for a full summary of all of the data providers.
...
•	Data undergo a coded QA/QC process using R-based scripting.
•	Initial automated routines are performed to make sure data conform, in the most basic sense, to the Program's data schema, aka data dictionary (see Appendix 2). 
...
–	The data are evaluated regarding how well they conform to the specific valid values listed in the data dictionary (see Appendix 2 for the full data dictionary).
–	Taxonomic problems with any 'ScientificName' are identified and corrected based on the World Register of Marine Species (WoRMS).
...
–	A visual summary referred to as a QA dashboard is generated using R Markdown Language. The dashboard visualizes the values in each field in such a way as to summarize and highlight possible issues. The dashboard report maps the positions of all occurrences.
...
The data schema, published in 2015, has remained stable, but changes have been made to enhance the utility of the database, and to improve clarity of the field definitions set forth in Hourigan et al., 2015. The full data dictionary for the National Database contains 133 fields, of which 108 are published and 25 are unpublished and used internally by the Program for data management purposes. For a discussion of the required and desired fields for submitting data to the Program, please see Appendix 3.
Logical groupings of fields, called 'Field Category', reflect the higher level organization of the database and provide a convenient way to group related fields. These groups demonstrate how the database fields relate to one another and describe different aspects of the individual occurrences (or observations) of corals and sponges. Two new categories, “GIS Enhancement”, and “Flagging ” have been added to capture a suite of new fields, bringing the number of field categories from eight to ten (Table 2).
...
•	Quality Flags: Published quality and usability flags defined by the Program.
For a full list of current field names in the database, their definitions, and their valid values, see Appendix 2 of this document. The data dictionary for the program is a version-controlled document (20190718-0 at the time of writing). Appendix 2 is arranged alphabetically to assist the reader when searching for specific fields. The field definitions and valid values are periodically updated by the Program's Data Working Group. The data dictionary presented in the previous technical memo, Hourigan et al. (2015), is superseded by the current version. For the most current version of the data dictionary, users should consult the DSCRTP web portal at: https://deepseacoraldata.noaa.gov/internal-documents/program-guidance/science-team-guidance-for-data-management/20170707.xlsx/view . The definitions posted at that location will supersede all other versions (including the one published in this document). 




Table 2: Public fields of NOAA's National Database for Deep-Sea Corals and Sponges grouped by 'Field Category'.  The single quoting convention used in this document is not used in the table below for enhanced visual clarity.


Several additions of internal and external fields, updates to field definitions, and updates to valid values have occurred since 2015. The Program recognizes the need for field definitions to remain as stable as possible through time, but the field definitions and valid values are revisited periodically to make additions or changes, to capture critical new incoming information, or to clarify and refine the way a field should be implemented. Typical changes include adding a new field, improving the definition of an existing field, and changing the valid value set for an existing field. Below are key changes to the database fields that have occurred since 2015.  
Two new taxonomy-related fields were added.
•	VerbatimScientificName: The 'ScientificName' as originally reported by the 'DataProvider'.
•	TypeStatus: The type status (e.g., holotype, paratype, etc.), if any, of the specimen. Additional information (e.g., typified scientific name, publication) should be included in 'IdentificationComments'.  The database currently contains over 700 records of holotypes.
...
•	VerbatimSize: This field is used to store the originally reported size value (in cm). If the record represents a size range (e.g., an estimate for an individual, or the range for a number of the same taxon of coral or sponge), then use of 'MinimumSize' and 'MaximumSize'
•	MinimumSize: Minimum size of the taxon observed. Methods for determining size (e.g., height or maximum diameter) should be noted in 'OccurrenceComments'.
•	MaximumSize: Maximum size of the taxon observed. Methods for determining size (e.g., height or maximum diameter) should be noted in 'OccurrenceComments'.
...
•	CMECSGeoForm: Tectonic or geologic features, such as shelf, canyon, or seamount. See the Coastal and Marine Ecological Classification Standard catalog for description. 
•	CMECSSubstrate: Geologic or biogenic substrate. See the Coastal and Marine Ecological Classification Standard catalog for description 
•	CMECSBiotic: Biotic community present at the observation site. See the Coastal and Marine Ecological Classification Standard catalog for description.
...
•	EntryUpdate: The most recent date the DSCRTP Program updated or verified the record. Format: YYYY-MM-DD. This variable will only contain a value if the DSCRTP Program has updated a record which has previously been published in the database. Also see the related variables that inform record versioning: 'Modified' and 'EntryDate'.
...
•	ShallowFlag: A value of 1 denotes a coral or sponge record where DepthInMeters is correct but it is less than 50 meters. A value of 0 denotes a coral that has a correct DepthInMeters greater than or equal to 50 meters.
Several new spatially calculated depth fields were added to help ground truth the observed depths within the database.
•	gisCRMDepth: GIS-derived depth (bathymetry is Coastal Relief Model).
•	gisGEBCODepth: GIS-derived gisGEBCODepth (bathymetry is GEBCO).
•	gisEtopoDepth: GIS-derived depth (bathymetry is ETOPO1).
The following location related fields were added to enhance the ability to describe and standardize place names.
•	gisNGIALocality: GIS-derived Locality (based on NGIA point placenames) - 30km max search distance for matching.
•	gisNGIADist: distance to nearest official NGIA locality (km).
•	gisGEBCOLocality: GIS-derived Locality (based on GEBCO point placenames) - 30km max search distance for matching.
•	gisGEBCODist: distance to nearest official GEBCO locality (km).
The following fields have been added to capture the actual spatially explicit boundary of the sample area footprint for each record.
•	footprintWKT: A Well-Known Text (WKT) representation of the shape (footprint, geometry) that defines the sample area. As an example, for downward looking or oblique still images, this would be a polygon representing the footprint of the viewable area (an orthogonal projection using the sea floor as the projection plane). For a video transect, this would be a polygon that represents the footprint of the entire viewable area throughout the length of the video. To report the actual area of the sample, see 'SampleAreaInSquareMeters'. Also please report 'footprintSRS' if you report 'footprintWKT'. Example: the one-degree bounding box with opposite corners at (longitude=10, latitude=20) and (longitude=11, latitude=21) would be expressed in well-known text as “POLYGON ((10 20, 11 20, 11 21, 10 21, 10 20))” withouth the enclosing quotes. For discussion see http://terms.tdwg.org/wiki/dwc:footprintWKT

•	footprintSRS: A Well-Known Text (WKT) representation of the Spatial Reference System (SRS) for the footprintWKT of the Location. Do not use this term to describe the SRS of the Latitude and Longitude, even if it is the same as for the footprintWKT. 'footprintSRS' must always be populated if 'footprintWKT' is given. Example: The WKT for the standard WGS84 SRS (EPSG:4326) is “GEOGCS["GCS_WGS_1984",DATUM["D_WGS_1984",SPHEROID["WGS_1984",6378137,298.257223563]],PRIMEM["Greenwich",0],UNIT["Degree",0.0174532925199433]]” without the enclosing quotes. For discussion see http://terms.tdwg.org/wiki/dwc:footprintSRS
...
Connection between the Ocean Biogeographic Information System, USA (OBIS-USA) and the National Database

The Program is committed to contributing to national and international aggregations of occurrence data. To this end, the Program contributes updated coral and sponge data on an annual basis to OBIS-USA, the U.S. node of OBIS which is part of the International Oceanographic Data and Information Exchange (IODE) of the Intergovernmental Oceanographic Commission (IOC) of the United Nations Educational, Scientific and Cultural Organization (UNESCO). The National Database contributes a particular subset of fields from its database to OBIS-USA during annual submissions and these fields align to the Darwin Core standard, the standard used by OBIS-USA (Table 3).  





Table 3: Fields from the National Database for Deep Sea Corals and Sponges that are contributed to the Ocean Biogeographic Information System – USA (OBIS-USA).  The vocabulary crosswalk table below is in alphabetical order by DSCRTP field names.  The corresponding Darwin Core term is given that matches the term used within OBIS.  The OBIS-required fields are highlighted.

Note to Reviewers: This table is currently under review by our data working group.


Summary of Database Contents
The following figures and statistics provide insight into the completeness and quality status of datasets included within the National Database. These results are meant to inform and caution data users about the adequacy of using the database for various analytical objectives. Appendix 4 includes a link to an online resource that describes each unique data set in detail as well as providing an interactive map to explore those data in isolation.  The dataset dashboards also have a link to download the dataset as well as International Standards Organization compliant metadata (ISO 19115-2).
...
Analysis Details
The analyses presented here use version 20190718-0 of the database. In some cases, the data have been filtered to enhance readability or interpretation of a figure or table. These filters will be called out explicitly in the text or captions when used. The purpose of this analysis is to succinctly summarize the current contents and quality of the database, so that any major limitations are apparent to end. The numerical and graphical summaries provided in this document were created using R and R-Markdown used within the RStudio development environment (R Core Team, 2017; RStudio Team, 2016). The author can provide the code upon request.
Distribution of Records by U.S. Fishery Management Council Region
Primary clients for the National Database are the United States Regional Fishery Management Councils. The 'FishCouncilRegion' field identifies the council region within which a record occurs. Figure 2 represents the number of sponge and coral occurrences that fall within each of council regions. Around 5 % of records are not found within U.S. Fishery Management Council regions, as they are either within international waters or within the boundaries of other countries' exclusive economic zones.





Figure 2: Distribution of known coral and sponge occurrences (as of 2018) reported by Fishery Management Council region ('FishCouncilRegion'). Only records occurring within a council boundary are included. The number of records is primarily a measure of sampling effort and reporting to the database, not necessarily a reflection of the abundance of deep sea corals or sponges within that region.  Records outside the U.S. EEZ are not included in this figure.
Images Associated with Records
To date, approximately 7 % or 50,498 records have full resolution still images available. These images are viewable directly in the DSCRTP map portal by clicking on coral or sponge points. The 'ImageURL' field also contains the link to the image if available, and this field is available in data downloaded from the DSCRTP map portal.
Depth Distribution
'DepthInMeters' is the primary depth field in the database with all of the published records including depth information in this field. This field records either the actual reported depth, the average depth (when separate 'MinimumDepthInMeters' and 'MaximumDepthInMeters' are reported), or, in cases where no depth is reported, a GIS-derived depth for their position is assigned, relative to the best available bathymetry. These spatially derived depths are available in the following fields: 'gisCRMDepth', 'gisGEBCODepth', and 'gisEtopoDepth' and provide an important means of checking depth.  Averaged and assigned depths should be treated as approximations. 
The 'DepthMethod' and 'LocationComments' fields contain additional details on the source and methodology for depth values present in the 'DepthInMeters' field. For full definitions and valid values of these fields, see Appendix 2. See Figure 3 below for a distribution of depth values in the National Database by Fishery Management Council region ('FishCouncilRegion'). It is important to note here again that not all areas have been surveyed and thus these values only represent the currently held occurrences. 

Figure 3: Distribution of depth values by Fishery Management Council region ('FishCouncilRegion'). The 'NA' values are records that are outside of the United States exclusive economic zone.
Observations through Time
The year in which an observation was made, 'ObservationYear', provides some insights for understanding deep sea exploration activities through time. Figure 4 provides a visualization of the distribution of the 'ObservationYear' field. The database includes records from 1833 to 2017, with the median year of observation being 2007.  Most records prior to 1980 represent museum specimens.  The large increase in 21st century records reflects the increasing importance of records annotated from still images and video.

Figure 4: The temporal distribution of coral and sponge observations by 'ObservationYear.'
 
Individual Count
The 'IndividualCount' field indicates the number individual corals or sponges per record. 64 % or 445,859 records have 'IndividualCount' values. Most of the records fall within the range of 1-50 individuals per observation frame, but the maximum is over 16,000 individuals.  See Figure 5 for the distribution of individual count between 0 and 50.  
When interpreting individual count values, it is important to realize that some records represent linear sampling events, like a trawl or video transects and so the underlying sample area is much bigger, leading to higher overall individual count values.  Where 'SampleAreaInSquareMeters' is reported, this value can be used to spatially normalize the count values.  Also, when 'IndividualCount' is not populated,  species abundance values may be reported in 'CategoricalAbundance' instead, which is simply a categorical expression of the count of the number of individuals. 

Figure 5: Distribution of 'IndividualCount' values between 0 and 50 corals or sponges. The figure's x-axis is truncated at 50 individuals for visual clarity, but the true maximum for 'IndividualCount' is 16,791.
 
Size
For records with reported size values (23 % of all records; n = 157,997), all are properly specified according to the data dictionary. See Figure 6 below for distribution of average size values.  In the database, size values are reported in 'MinimumSize' and 'MaximumSize'fields in order to capture a range of sizes that might be observed within a sample area. In the case of a video transect, for example, multiple sizes may be observed for a single taxon. Average size values reported in the figure below are calculated as ('MinimumSize' + 'MaximumSize')/2.   Values for these two fields will be identical where only one size observation was made.  The original size values, exactly as reported by the data provider, are captured in a field called “VerbatimSize'. 


Figure 6:  Histogram of average size values. For clarity the x-axis limit has been set to 100 cm, but the maximum average size is 762 cm. 
 
Condition
For records with reported condition values (62 % of all records; n = 405,491), 99% comply with the list of valid values specified in the data dictionary. Most entries are listed as 'Live' (Figure 7).

Figure 7: Distribution condition values, showing most records in the database are of live coral and sponges.
Habitat and Substrate
Habitat and substrate values are in the database as reported by the data provider. No standard valid values have been adopted or implemented for these fields.  The newly-established fields using CMECS classifications (CMECSBiotic, CMECSGeoForm, CMECSSubstrate) are intended to help standardize reporting of habitat measures.
•	18 % or 127,708 records have 'Substrate' values reported.
•	39 % or 271,370 records have 'Habitat' values reported.




Taxonomy
All records in the database (100%) are populated with a complete taxonomic string. In addition, greater information about taxonomic identifications are available in the following descriptive fields: 'IdentificationComments',  'IdentifiedBy',  'IdentificationDate',  and 'IdentificationQualifier'. See Appendix 2 for the full definitions of these fields.
Taxonomic identifications can change through time, either through refinements in identification by taxonomists or through changes to underlying group taxonomy. DSCRTP attemps to keep up with these changes, but these fields should be checked carefully before using the data for analysis.
'TaxonRank' provides taxonomic resolution of the taxonomic identifications in the database. Figure 8 provides a visualization of the distribution of the 'TaxonRank' field. The bars are colored by 'Phylum' as an indication of the relative portion of corals vs. sponges and their taxonomic resolution. Most corals are identified to the taxon rank ('TaxonRank') of genus or species.  
...
See Appendix 1 for a full summary of the taxa in the database.  
Record Types
100 % of all records (n = 695,335) have 'RecordType' values reported. See Figure 9 below for a view of the distribution of records by 'RecordType' within each Fishery Management Council region. Figure 10 shows the distribution of these values across the entire database, regardless of region.



Figure 9: The distribution of 'RecordType' within the database for each 'FishCouncilRegion.'

Figure 10: The distribution of valid 'RecordType' values across the entire database.

Sampling Equipment
Approximately 98 % of all records (n = 678,773) have 'SamplingEquipment' values reported. Additional information on the sampling equipment (e.g., specific type of trawl) and other methods are often included in the 'SurveyComments' or in the data set metadata.  Figure 11 shows the distribution of 'SamplingEquipment' values, grouped by 'FishCouncilRegion' to identify the regional differences in sampling methodology. See Figure 12 below for the makeup of the entire database by 'SamplingEquipment'.





Figure 11: The distribution of 'SamplingEquipment' values, grouped by 'FishCouncilRegion'.


Figure 12: The distribution of 'SamplingEquipment' valid values across the entire database.
Caveats for Data Usage
...
Horton, T. et al. (2018) World Register of Marine Species. Available from http://www.marinespecies.org at VLIZ. Accessed 2018-03-10. doi:10.14284/170.
...
Summary of all occurrence records in the database grouped and listed alphabetically by 'Family'.  The number of genera ('Genus') with records in the database within in each family is reported in the column named 'Genera' in the table below. For records listed only at the Family level, Genus will be “NA”.  The column heading '# of Records' represents the number of records within each family. 
...
National Database field names, definitions, and their valid values, organized by field name. The current version of this table is available as an Excel file on the DSCRTP web portal: 
...
Appendix 3: Required and Desired Fields for Submission of Data to the National Database
...
Used in combination, these factors (each with three categories), lead to nine different types of incoming data with different numbers of required fields. The Data Working Group uses notation to refer to these combinations: Point Historical, Point New, Point Program, Transect Historical, Transect New, Transect Program, Trawl Historical, Trawl New, Trawl Program. Tables 4a-c below summarize the required fields for the nine possible incoming data types.
Table 4a: Required and desired fields for different types of incoming point-based data. 'R' means the field is required for data submission and 'D' means that it is desired by the Program. 'P' means that the field is supplied by the Program.


Table 4b: Required and desired fields for different types of incoming transect-based data. 'R' means the field is required for data submission and 'D' means that it is desired by the Program. 'P' means that the field is supplied by the Program.
...
Table 4c: Required and desired fields for different types of incoming trawl-based data. 'R' means the field is required for data submission and 'D' means that it is desired by the Program. 'P' means that the field is supplied by the Program.


Appendix 4: Data set Dashboards
Each of the data sets in the database has a unique 'DatasetID'. There are currently 695,335 records in the database composed of 226 individual data sets from 75 different data providers. You will find an interactive dashboard for each data set at the following location: https://deepseacoraldata.noaa.gov/DatasetID_Table/DatasetID_Table_20190418-0.html These dashboards provide important information about each constituent data set that makes up the National Database and to download each set individually with ISO compliant metadata.

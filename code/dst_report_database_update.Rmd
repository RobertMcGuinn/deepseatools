...
## no need to change these here, they will get overwritten by the  parameters listed in the runner
params:
  quarter: "Q4, FY-2025"
...
```{r calculations, echo=FALSE, cache=F}
...
##### transformation: make all -999 values in InvidualCount = 1.  This is a simplifying ##measure for ease of counting individual observations. This mutated 'IndividualCount' should not be used for analysis.
...
min_obs_date <- min(as.Date(x$ObservationDate))
max_obs_date <- max(as.Date(x$ObservationDate))
...
## New Database Version: `r params$version`

The Deep Sea Coral Research and Technology Program is proud to announce that the latest version of NOAA's National Database for Deep Sea Corals and Sponges is now [live](https://www.ncei.noaa.gov/maps/deep-sea-corals-portal/). **`r prettyNum(length(x$CatalogNumber), big.mark = ',')`** new records were added this round, for a total **`r prettyNum(length(filt$CatalogNumber), big.mark = ',')`** occurrence records in the database. Please see the following tables and figures for additional detail regarding this update.  

Visit our portal to view and download custom data subsets of your choosing:
...
* Number of new database records added: **`r  prettyNum(length(x$CatalogNumber), big.mark=",")`**

* Individual coral and sponge observations added: **`r prettyNum(sum(x$IndividualCount[x$IndividualCount != "-999"]), big.mark = ',')`**

* Number of EventID(s) (number of dives/trawls/transects) added: **`r prettyNum(length(unique(x$EventID)), ',')`**

* Observation time frame of added records: **`r min_obs_date` to `r max_obs_date`**

* Publication date on map portal: **`r params$releasedate`**

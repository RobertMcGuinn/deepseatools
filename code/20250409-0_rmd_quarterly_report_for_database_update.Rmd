...
## [Manual]
quarter <- 'Q2, FY-2025'
version <- 'DSCRTP_NatDB_20250409-0'
...
# patch3 <- read.csv('../indata/VerbatimScientificName_patch.csv')
# 
# x <- patch1$CatalogNumber
# y <- patch2$CatalogNumber
# z <- patch3$CatalogNumber
# unique_catalog_numbersunique(c(x, y, z))

## check 
# filt %>% filter(CatalogNumber %in% unique_catalog_numbers, Class == 'Anthozoa') %>% 
#   pull(CatalogNumber)
# 
# filt %>% 
#   filter(CatalogNumber == '195534') %>%
...
```{r load_both_db, echo=FALSE, cache=T, eval=F}
##### [Manual] bring last two databases (old one first)
setwd("C:/rworking/deepseatools/indata/DSCRTP_NatDB_20241219-1_CSV")
indata1<-read.csv("DSCRTP_NatDB_20241219-1.csv", 
                  header = T, 
                  encoding = 'latin9')
filt1 <- indata1 %>%
  filter(Flag == "0")
...
indata2<-read.csv("DSCRTP_NatDB_20250409-0.csv", 
                  header = T, 
                  encoding = 'latin9')
filt <- indata2 %>%
  filter(Flag == "0")
...
```{r load_test, echo=FALSE, cache=F}
# rm(indata1)
# rm(indata2)

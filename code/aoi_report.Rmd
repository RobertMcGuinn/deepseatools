---
title: "NOAA National Database for Deep-sea Corals and Sponges: Area of Interest (AOI) Summary for Derek Sowers"
...
##### create subset from bounding box from Derek Sowers ###### 
...
## write data to Excel
# setwd("C:/rworking/deepseatools/indata")
# write.xlsx(x,"DSCRTP_NatDB_20200408-1_subset_DSowers_bounding_box_RPMcGuinn.xlsx", 
#            row.names = F, quote = T)
...
## write data to Excel
setwd("C:/rworking/deepseatools/indata")
write.xlsx(x,"20200624-0_subset_of_DSCRTP_NatDB_20200408-1_FMC_South_Atlantic_RPMcGuinn.xlsx",
           row.names = F, 
           quote = T)
...
# Growth Through Time within AOI
...
subx <- x %>% filter (Phylum == "Cnidaria", FishCouncilRegion == region) %>%
...
suby <- x %>% filter (Phylum == "Porifera", FishCouncilRegion == region) %>%
  mutate(EntryDate = replace_na(as.character(EntryDate), '2019-12-18')) %>% 
  mutate(EntryDate = as.factor(EntryDate)) %>% 
  mutate(as.Date(EntryDate))

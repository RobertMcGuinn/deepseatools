...
```{r filter_query, echo=F, warning=F, message=F, eval=T}
x <- filt %>%  
  filter(grepl("Clarke", PI))#| 
           #grepl("NWFSC", DatasetID))

## check
# x %>% pull(DatasetID) %>% table()
# x %>% pull(SurveyID) %>% table()
...
ggplot(subx,aes(x=as.Date(EntryDate), color=Phylum)) +
  ...
  ggtitle(unique(filt$DatabaseVersion))

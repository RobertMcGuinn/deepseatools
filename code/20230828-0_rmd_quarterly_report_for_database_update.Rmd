...
## [Manual]
quarter <- 'Q4, FY-2023'
version <- 'DSCRTP_NatDB_20230828-0'
...
indata1<-read.csv("DSCRTP_NatDB_20230620-0.csv", 
                  header = T, 
                  encoding = 'latin9')
filt1 <- indata1 %>%
  filter(Flag == "0")
...
##### bring new db in #####
setwd("C:/rworking/deepseatools/indata")
indata2<-read.csv("DSCRTP_NatDB_20230928-0.csv", 
                  header = T, 
                  encoding = 'latin9')
filt <- indata2 %>%
  filter(Flag == "0")
...
newdatasetIDs <- filt %>% 
  filter(as.Date(EntryDate) > as.Date('2023-08-01')) %>%  
  pull(DatasetID) %>% 
  unique()

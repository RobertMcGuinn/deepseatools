...
```{r OR_data_and_subset_local, echo=FALSE, message=FALSE, warning=FALSE, eval=T}
...
```{r OR_data_and_subset_gdrive,echo=FALSE, message=FALSE, warning=FALSE, eval=F}
##### ##### 
...
### AccessionID 
Target Accession ID: `r substring(filename,12)`
...
* Time frame: *`r min(sub$ObservationDate)` to `r max(sub$ObservationDate)`*
* Number of ImageURL populated: `r sub %>% filter(is.na(ImageURL) == F) %>% pull(ImageURL) %>% length()`
* Number of ImageFilePath Populated: `r sub %>% filter(is.na(ImageFilePath) == F) %>% pull(ImageFilePath) %>% length()`   

# Citation
``` {r Citation, echo=FALSE, eval = T}
sum_tbl <-
  sub %>%
  group_by(Citation) %>%
...
sub$CitationMaker <- paste(sub$DataProvider,'. ','Observation date range: ', 
                         min(sub$ObservationDate[sub$ObservationDate != "-999"]),' to ',
                         max(sub$ObservationDate[sub$ObservationDate != "-999"]),'. ',
...
##### check ##### 
# sub %>% filter(Vessel == 'Meiring Naud\xe9 R/V') %>% dim()
...
`r min(sub$ObservationDate, na.rm=TRUE)` to `r max(sub$ObservationDate, na.rm=TRUE)`
...
`r x <-sub %>%
  filter(is.na(as.Date(ObservationDate, format = "%Y-%m-%d")) == F) %>%
           group_by(DatasetID) %>% 
           summarize(n=n())
...
## Depth Method Summary
``` {r DepthSumByMethod, echo=FALSE}
sum_tbl <- data.frame(table(sub$DepthMethod, useNA = 'always'))
names(sum_tbl) <- c('Depth_Method', 'Frequency')
...
## Depth Summary by ScientificName (Fish)

``` {r DepthSumByScientificNameFish, echo=FALSE}
sum_tbl <-
  sub %>%
  filter(Phylum == "Chordata") %>%
  group_by(ScientificName) %>%

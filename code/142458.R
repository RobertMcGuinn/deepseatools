...
filename <- "20250321-3_NOAA_CBNMS_ROV_2017_142458"
...
sub2 <- sub1  %>%
  mutate(ScientificName = trimws(ScientificName),  # Remove extra spaces
         ScientificName2 = case_when(
           ScientificName == "Chromoplexaura spp." ~ "Chromoplexaura",
...
##### load data #####
setwd('c:/rworking/deepseatools/indata')
filename <- '20250402-0_NOAA_CBNMS_ROV_2017_142458'
sub <- read.csv(paste(filename, '.csv', sep = ''))
...
folderurl <- "https://drive.google.com/drive/folders/15Yl4iAKFDqKrMI_NQmqmtv9WrgwfUTPk"

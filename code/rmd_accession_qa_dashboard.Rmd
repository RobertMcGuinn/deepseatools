...
opts_chunk$set(fig.width=7, fig.height=3.5, dpi = 300)
```

```{r data_and_subset, echo=FALSE, message=FALSE, warning=FALSE}

# # if you want to use a specific dataset
# dataset <- 'DWH_NRDA_WS-14-06' #'NOAA_M2-10-06_AUV'
# sub <- indata %>% filter(grepl(dataset, DatasetID))
...
sum(x$n)`
* Number of ImageFilePath Populated: 
`r x <- sub %>%
...
ft <- flextable(sum_tbl) %>% 
  ...
  autofit()  %>% 
  width(j = ~ DataProvider + Reporter + ReporterEmail + DataContact + Records, width = 7.5/length(names(sum_tbl)))
...
ft <- flextable(sum_tbl) %>% 
  ...
  fontsize(size = 7, part = "all") %>%
  autofit()  %>% 
  width(j = ~ SurveyID + Vessel + VehicleName + SamplingEquipment + NavType + LocationAccuracy + ObservationDates, width = 7.5/length(names(sum_tbl)))
...
ft <- flextable(sum_tbl) %>% 
  ...
  fontsize(size = 7, part = "all") %>%
  autofit()  %>% 
  width(j = ~ Family + ScientificName + TaxonRank + UnFlagged + FlagReason + Records, width = 7.5/length(names(sum_tbl)))
...
Nothing will appear here if no 'Density' values are present
...
If name appears below after variable, then it does not match the standard name.
...
# SurveyIDs for the same Vessel
``` {r standardized_SurveyID_check, echo=FALSE, cache = FALSE}

SurveyIDPattern <- filt %>% filter(Vessel %in% unique(sub$Vessel)) %>% group_by(DatasetID, Vessel, SurveyID, ObservationDate, PI) %>% summarize(n=n())
sum_tbl <- kable(SurveyIDPattern, row.names = F, digits = 2)
sum_tbl

```

# DatasetIDs for the same Vessel
``` {r standardized_DatasetID_check, echo=FALSE}

DatasetIDPattern <- filt %>% filter(Vessel %in% unique(sub$Vessel)) %>% group_by(DatasetID, Vessel, SurveyID, ObservationDate, PI) %>% summarize(n=n())
sum_tbl <- kable(DatasetIDPattern, row.names = F, digits = 2)
sum_tbl

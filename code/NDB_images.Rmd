...
x <- images_yo %>% filter(Width == "1920") %>% pull(Name)

name <- filt %>% filter(grepl(as.character(x[10]), ImageURL))

yo <- filt %>% filter(grepl(as.character(x[10]), ImageURL)) %>% 
  pull(ImageURL)

browseURL(as.character(yo))

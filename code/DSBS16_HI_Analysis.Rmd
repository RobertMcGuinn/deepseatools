...
write.xlsx(x, "20210820-0_species_lists_per_EventID.xlsx")
...
## write it to excel 
filename = "20210823-2_Summary_of_all_corals_and_sponges_by_ScientificName_with_groups_Scientific_EventID.xlsx"
setwd("C:/rworking/deepseatools/indata/")
...
z$DepthCat[as.numeric(z$DepthInMeters) <= 2000] <- "shallow"
z$DepthCat[as.numeric(z$DepthInMeters) > 2000] <- "deep"
z$DepthCat <- factor(z$DepthCat, levels = c("shallow", "deep"))

##### setting location categories ##### 
z$loc[as.numeric(z$Latitude) > 25] <- "north"
z$loc[as.numeric(z$Latitude) < 25] <- "south"
...
##### longitude cat #####
z$longloc[as.numeric(z$Longitude) > -168] <- "east"
z$longloc[as.numeric(z$Longitude) < -168] <- "west"
...
library(vegan)
NMDS <- metaMDS(site.sp, distance = "jaccard", binary = T, k=3, trymax = 30)
...
shannon_diversity <- diversity(site.sp, index = "shannon") #makes an object
...
# creating plots
plots_df <- data.frame(shannon_diversity)
# View(plots_df)
...
x <- d %>% group_by(EventID, shannon_diversity) %>% summarize(n=n()) %>% #, 
...
setwd("C:/rworking/deepseatools/indata")
write.xlsx(x, '20210823-0_Summary_of_Corals_and_Sponges_Diversity_by_EventID.xlsx') 

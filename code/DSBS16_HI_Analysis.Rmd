...
The R Markdown code used to generate this report is available here: [LINK](https://github.com/RobertMcGuinn/deepseatools/blob/master/code/DSBS16_HI_Analysis.Rmd)  Comments and code within R Markdown document further elucidate the procedures being used. Having this code available will facilitate the reanalysis of these data. 

## Input Data

Datasets used in this analysis were acquired on 2021-08-18 from Tom Hourigan via Excel file. The coral and sponge species occurrence records are from a subset of dives in the Hawaiian region (including Johnston Atoll and the Musician Seamounts). This original dataset is shared on Google Drive here: [LINK](http://docs.google.com/spreadsheets/d/16izQ_vWsSDgMupbNAEUsVTvMISzmrLaC/edit?usp=sharing&ouid=109414727136135095326&rtpof=true&sd=true)

Tom Hourigan issued some corrections, via Excel file, to the above dataset on 2021-08-26: [LINK](https://docs.google.com/spreadsheets/d/1Rg4fUrvO_0VT2TVNNANPKGbMyxg-jh2p/edit?usp=sharing&ouid=109414727136135095326&rtpof=true&sd=true) and those changes were incorporated prior to this analysis.  
...
rect.hclust(fit, k=4, border="red", cluster = groups)
...
class(site.scores)
names(site.scores)
test <- merge(site.scores, df)
site.scores <- test
site.scores$groups <- as.factor(site.scores$groups)
# yo <- c(fit$labels)
# what <- c(fit$order)
# df <- data.frame(yo, what)

```

### Plot of NMDS with clusters on top
...
ordiellipse(NMDS, groups, col=c("blue", "green", "red", "black"), draw="polygon")
...
```{r plotting_NMDS_cluster_groups}
##### plotting NMDS ##### 
ggplot() +
  geom_point(data=site.scores,aes(x=NMDS2,y=NMDS1, color=groups), size=5) + # add the point markers

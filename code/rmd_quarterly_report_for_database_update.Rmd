...
quarter <- "Q2, FY-2019"
version <- "DSCRTP_NatDB_20190418-0"
...
```{r load big data, echo=FALSE, cache=TRUE}
##### Load Global Dataset ##### 
# indata <- read.csv(paste("C:/rworking/digs/indata/", paste(version,".csv",sep=""), sep = ""), header = T)
# indata<- indata %>% filter(Latitude != "-999", Longitude != "-999", Flag == "0")

```

```{r filter_big_data, echo=FALSE, cache=FALSE}
# this is used to filter out only those records with an EntryDate later than some date

x <- indata %>% 
  filter(
    Flag == "0", 
    as.Date(EntryDate) > '2019-04-01'
  )

# check
# table(factor(x$DatasetID))
# yo <- table(factor(x$DatasetID))
# length(yo)
# 
# table(factor(x$EntryDate))
# yo <- table(factor(x$EntryDate))
# length(yo)
...
*A big thank you to all of our data providers! We hope this database is providing a needed service to deep sea coral and sponge conservation. We welcome your questions and feedback.*
...
g <- ggplot(x, aes(Order)) +
 ...
 # coord_flip() + 
  ...
  theme_bw(base_size = 9, base_family = "Cambria")
...
sum_tbl <-
  ...
  summarize(DatasetID = toString(unique(DatasetID)), 

...
indata<-read.csv("DSCRTP_NatDB_20190418-0.csv", header = T) #DSCRTP_NatDB_20181005-0.csv # DSCRTP_NatDB_20181211-2.csv
filt <- indata %>%
  filter(Flag == "0")
...
key.new <- gs_title('20190415-0_DatasetID_Key_DSCRTP')
#gs_browse(key.new)
...
setwd("C:/rworking/digs/indata")
int <- read.csv("intersect.csv", header = T)
...
setwd("C:/rworking/digs/indata")
sub <- read.table("BioODV_Demfish_multiCDI.txt", header = T, sep="\t", fill = TRUE)

##### _____ get modified and original datasets via Excel ##### 
setwd("C:/rworking/digs/indata")
alt <- read.xlsx('20190301-0_Hourigan_Q2_NewDatasets_THourigan.xlsx', sheet = 1)
...
##### write schema #####
setwd("C:/rworking/digs/indata")
write.csv(s, "2018_DSCRTP_Schema.csv")
...
x <- s %>%
  filter(
    grepl("SamplingEquipment", FieldName)
  ) %>%
  group_by(
    FieldName
  ) %>%
...
setwd("C:/rworking/digs/outdata")
for (name in levels(factor(indata$DatasetID))){
  ...
  fn <- paste('xls/','20190208_0_',gsub(' ','',name),'.xlsx',sep='')
...
##### DatasetID queries #####
dataset <- 'DWH_NRDA_HC-11-09' #'DWH_NRDA_HC-11-09', 'DWH_NRDA_NF-10-14', 'DWH_NRDA_WS-14-06'

ndb <- indata %>% filter(
  grepl(dataset, DatasetID)#,
  #is.na(SampleID) == T
                         ) %>% 
  group_by(#Flag,FlagReason, 
           DatasetID, Citation,
           AccessionID, SurveyComments, Locality, Vessel, SurveyID,
  ...
  summarize(n=n(),
            MinimumDepthInMeters = min(DepthInMeters),
            MaximumDepthInMeters = max(DepthInMeters)
            )
...
yo <-  filt %>% filter(grepl(fc, FishCouncilRegion)) %>% 
  group_by(Locality, Vessel, SurveyID, EventID, 
           VehicleName, ObservationYear, 
           ObservationDate, DataProvider, PI, 
           Reporter, DataContact, DatasetID, 
